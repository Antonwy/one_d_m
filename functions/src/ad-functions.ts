...
exports.onAdDocChanged = functions.firestore
    .document(`${DatabaseConstants.user}/{userId}/${DatabaseConstants.advertising_data}/${DatabaseConstants.ad_impressions}`)
    .onWrite(async (snapshot, context) => {
        const userId: string = context.params.userId;

        const nativeAdImpressions: number = (snapshot.after.data() as any)[UserAdFields.native_ad_impressions];
        const interstitialImpressions: number = (snapshot.after.data() as any)[UserAdFields.interstitial_impressions];

        const moneyGeneratedSinceLastDC: number = (nativeAdImpressions || 0) * (1.4 / 1000) + (interstitialImpressions || 0) * (2.1 / 1000);
